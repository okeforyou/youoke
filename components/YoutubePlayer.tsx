...
// import { useGoogleCast } from "../context/GoogleCastContext"; // REMOVED
...
function YoutubePlayer({
  ...
  // Google Cast
  // const { isCastAvailable: isGoogleCastAvailable, requestSession } = useGoogleCast(); // REMOVED: Conflict with CastContext
            ...
            {!isCasting ? (
              <div className="space-y-3">
                ...
                <div className="relative flex items-center py-2">
                  ...
                  <span className="flex-shrink-0 mx-4 text-xs font-medium text-gray-400">หรือ</span>
                  <div className="flex-grow border-t border-base-300"></div>
                </div>

                <button
                  className="w-full py-2.5 px-4 text-base-content rounded-lg border border-base-300 hover:bg-base-200 font-medium flex items-center justify-center gap-2 transition-colors text-sm"
                  onClick={() => connectGoogleCast(playlist, roomCode)}
                  disabled={!isCastAvailable}
                >
                  <svg className="w-5 h-5 text-primary" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M1 18v3h3c0-1.66-1.34-3-3-3zm0-4v2c2.76 0 5 2.24 5 5h2c0-3.87-3.13-7-7-7zm0-4v2c4.97 0 9 4.03 9 9h2c0-6.08-4.93-11-11-11zm20-7H3c-1.1 0-2 .9-2 2v3h2V5h18v14h-7v2h7c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2z" />
                  </svg>
                  {isCastAvailable ? 'Cast ขึ้นจอ TV (Google Cast)' : 'กำลังโหลด...'}
                </button>
              </div>
            ) : (
              <div className="space-y-3">

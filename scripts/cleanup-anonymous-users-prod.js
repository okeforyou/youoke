...
// PRODUCTION - Use Production Firebase service account
const serviceAccount = require(path.join(__dirname, '../playokeforyou-firebase-adminsdk-6dzxn-984dbe3f20.json'));
...
const DAYS_THRESHOLD = daysArg ? parseInt(daysArg.split('=')[1]) : 7; // Default 7 days

async function cleanupAnonymousUsers() {
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - DAYS_THRESHOLD);

    console.log(`\n🔥 PRODUCTION Firebase Cleanup`);
    console.log(`⚠️  Target: playokeforyou (PRODUCTION)`);
    console.log(`📅 Cutoff Date: ${cutoffDate.toISOString()}`);
    console.log(`⏳ Threshold: ${DAYS_THRESHOLD} days`);
    if (DRY_RUN) {
        console.log(`✅ DRY RUN MODE: No changes will be made\n`);
    } else {
        console.log(`⚠️  *** LIVE MODE: Will delete users! ***\n`);
    }
    ...
    try {
        console.log('🔍 Scanning Production users...\n');
        ...
        console.log(`   Old Anonymous (>${DAYS_THRESHOLD} days): ${userIdsToDelete.length}`);
        ...
        if (userIdsToDelete.length > 0) {
            if (DRY_RUN) {
                ...
                console.log(`   Remaining Total:     ${totalCount - userIdsToDelete.length}`);
                console.log(`   Real Users:          ${totalCount - anonymousCount}`);
                console.log(`   Recent Anonymous:    ${anonymousCount - userIdsToDelete.length}`);

            } else {
                console.log(`\n🔐 Deleting ${userIdsToDelete.length} anonymous users from Production Auth...`);
                ...
                console.log(`\n🎉 Total deleted from Auth: ${totalDeleted}`);
        ...
        console.log('\n' + '='.repeat(60));
        console.log(`✅ PRODUCTION Cleanup ${DRY_RUN ? 'Analysis' : ''} Complete!`);
        console.log(`📊 Users ${DRY_RUN ? 'identified for deletion' : 'deleted'}: ${userIdsToDelete.length}`);
        if (!DRY_RUN) {
            console.log(`📊 Remaining users: ${totalCount - userIdsToDelete.length}`);
        }

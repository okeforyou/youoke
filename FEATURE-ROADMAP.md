...
### **Phase 6: Google Drive Personal Sync (Future Tech) ☁️**
เป็นแผนสำหรับการรองรับผู้ใช้งานมือถือ (Cross-Platform) แบบประหยัดงบและเลี่ยงปัญหาลิขสิทธิ์:
- ให้ Plugin บนคอมพิวเตอร์อัปโหลดไฟล์เพลงที่แยกเสียงแล้วไปยัง **Google Drive ส่วนตัว** ของผู้ใช้อัตโนมัติ (โฟลเดอร์ YouOke)
- หน้าเว็บ Web App บนมือถือ จะดึงข้อมูลไฟล์เสียงจาก Google Drive มาเล่น (สตรีมมิ่ง)
- ใช้ OAuth scope `drive.file` เพื่อความปลอดภัย (แอปเห็นเฉพาะไฟล์ที่ตัวเองสร้าง)
- **ข้อดี:** Zero Cost (ไม่ต้องเสียค่าเซิร์ฟเวอร์เก็บเพลง), Safe Harbor (ถูกกฎหมายเพราะเป็นการใช้งานส่วนบุคคล)

---

### **Phase 7: Advanced Multi-track Separation (Pro Feature) 🎛️**
เก็บไว้เป็นฟีเจอร์พรีเมียม (Pro) สำหรับผู้ใช้ระดับ Advance (Mixer แบบเดียวกับ Moises.ai):
- เพิ่มทางเลือกให้ผู้ใช้เลือกระดับความละเอียดการแยกเสียง AI
  - **Basic Mode:** แยกแค่ 2 แทร็ก (เสียงร้อง / ดนตรีรวม) - *ประมวลผลเร็ว*
  - **Advance Mode (Pro):** แยก 4 แทร็กเต็มระบบ (เสียงร้อง / กลอง / เบส / ดนตรีอื่นๆ) - *ประมวลผลนานขึ้น แต่ควบคุมได้ละเอียด*
- เปลี่ยน UI เป็น Mixer เต็มรูปแบบ สามารถปรับระดับเสียง (Fader), ปิดเสียง (Mute), หรือฟังเดี่ยว (Solo) ได้ทีละชิ้นดนตรี
- **ผลกระทบต่อระบบ:** แทบไม่มีผลกับระบบเดิม เพราะ Basic Mode ยังทำงานได้ไวตามปกติ ส่วน Advance Mode ทำงานผ่าน Demucs v4 (ที่มีอยู่ในระบบอยู่แล้ว) เพียงแต่ปลดล็อก `--two-stems=vocals` และรอประมวลผลนานขึ้น
- **เวลาในการพัฒนา:** ประมาณ 1-2 วันทำการ (เนื่องจากมี AI Engine รองรับอยู่แล้ว)

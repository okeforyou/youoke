...
export default function SpotifyDashboard({ showTab = true }) {
  ...
  return isError ? (
    ...
    <>
      ...
      {/* Loading State for Search (Genres) */}
      {
        isLoadingGenre && (
          <div className="col-span-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 px-2">
            {getSkeletonItems(5).map((s) => (
              <div key={s} className="h-10 bg-gray-50 rounded-xl animate-pulse" />
            ))}
          </div>
        )
      }

...
export default function SpotifyDashboard({ showTab = true }) {
  ...
  useEffect(() => {
    if (tempTopArtistsData && (genreText === "ลูกทุ่ง" || !genreText)) {
      setTopArtistsData(tempTopArtistsData);
    }
  }, [tempTopArtistsData, genreText]);


  const { data: artists, isLoading: isLoadingSongs } = useQuery({
  ...
  const {
    ...
    queryFn: ({ pageParam = 1 }) => searchPlaylists(genreText || "ลูกทุ่ง", pageParam),
  ...
  // Logic for artistCategories:
  // If we are on default "ลูกทุ่ง" and haven't loaded search results yet, show topArtistsData (V1 logic)
  // Else show search results
  const artistCategories = (genrePlaylists.length > 0)
    ? genrePlaylists
    : (genreText === "ลูกทุ่ง" ? (topArtistsData?.artistCategories || []) : []);
  ...
  return isError ? (
    ...
    <>
      ...
      <div ref={playlistRef} className="scroll-mt-32 col-span-full px-2 pt-4 pb-3 text-[13px] font-bold text-gray-500 uppercase tracking-wider flex items-center justify-between">
        ...
        {isLoadingGenre && <span className="loading loading-spinner loading-xs text-primary"></span>}
      </div>
      ...
      <div className="col-span-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6 px-4 pb-10">
        ...
        {isLoadingGenre && genrePlaylists.length === 0 && (
          <>
            {getSkeletonItems(6).map((s) => (
              <div key={s} className="relative w-full aspect-video rounded-3xl bg-gray-100 animate-pulse" />
            ))}
          </>
        )}
        ...
        {artistCategories.map((cat) => (
          ...
          >
            <Image
              ...
              unoptimized
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/99 via-black/20 to-transparent opacity-80" />
      ...
      {hasNextPage && (
        <div className="col-span-full flex justify-center pb-8">
          <button
            ...
            className="px-8 py-3 bg-white border border-gray-200 hover:border-primary/50 text-gray-500 rounded-full text-sm font-bold transition-all disabled:opacity-50"
          >
      ...
      {/* Song Grid */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 col-span-full px-2 pb-24">
        {isLoadingSongs && (
          <>
            {getSkeletonItems(12).map((s) => (
              <div key={s} className="w-full aspect-[4/3] bg-gray-100 rounded-lg animate-pulse" />
            ))}
          </>
        )}
        {artist?.map((video: any, i) => (
          <div
            key={video.id + i}
            className="group cursor-pointer bg-white rounded-lg border border-gray-100 hover:shadow-md transition-all active:scale-[0.98] relative overflow-hidden"
            onClick={() => {
          ...
          >
            <figure className="relative w-full aspect-[4/3] bg-gray-50 overflow-hidden">
              <Image
                src={video.imageUrl}
                ...
                className="object-cover transition-transform duration-700 group-hover:scale-110"
              />
              <div className="absolute inset-0 bg-black/20 group-hover:bg-black/10 flex items-center justify-center">
            ...
            <div className="p-2 flex flex-col gap-0.5">
              <h2 className="font-bold text-[11px] sm:text-[12px] line-clamp-1 text-gray-800">

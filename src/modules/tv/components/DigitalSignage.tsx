...
/**
 * YouOKE TV Digital Signage (v5.5.20)
 * Design: Leanback (YouTube-inspired TV Interface)
 * Optimization: Balanced layout with automatic template fallback
 */
export const DigitalSignage: React.FC<DigitalSignageProps> = ({
    ...
    // Force 'leanback' as default if template matches nothing or is empty
    ...
    const renderBackground = (opacity = 40) => {
        const items = template === 'ads' ? ads : displayImages;
        if (!items) return <div className="absolute inset-0 bg-stone-950" />;
            ...
            <div className="absolute inset-0 z-0 bg-stone-950">
                {items.map((item: any, index) => {
                    ...
                    if (type === 'video') {
                        return (
                            <video
                                ...
                                muted
                                loop
                                className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-[2000ms] ${isActive ? `opacity-[0.${opacity}]` : 'opacity-0'}`}
                            />
                        );
                    }
                    ...
                    return (
                        <div
                            key={index}
                            className={`absolute inset-0 bg-cover bg-center transition-all duration-[3000ms] ease-in-out ${isActive ? `opacity-[0.${opacity}] scale-100` : 'opacity-0 scale-110'}`}
    ...
    // --- LEANBACK / DEFAULT TEMPLATE ---
    if (template === 'leanback' || template === 'classic' || template === 'split' || !template) {
        return (
            <div className="absolute inset-0 bg-stone-950 overflow-hidden flex flex-col font-sans select-none text-white">
                ...
                <div className={`relative z-10 flex-1 grid grid-cols-12 h-full transition-all duration-1000 ${mounted ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
                    ...
                    <div className="col-span-5 flex flex-col justify-center items-center px-[5vw]">
                        <div className="w-full max-w-[32vw] aspect-[4/5] bg-white/5 backdrop-blur-[40px] border border-white/10 rounded-[3rem] p-[3vw] flex flex-col items-center justify-between shadow-2xl">
                            ...
                            <div className="relative group p-[1.5vw] bg-white rounded-[2.5rem] shadow-2xl">
                                <div className="w-[16vw] h-[16vw] min-w-[150px] min-h-[150px]">
                            ...
                            <div className="w-full space-y-[1vh] text-center">
                                ...
                                <p className="text-[0.9vw] text-white/30 pt-[0.5vh]">play.okeforyou.com</p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="relative z-20 h-[7vh] bg-stone-950/90 backdrop-blur-md border-t border-white/5 flex items-center">
                    ...
                    <div className="flex-1 overflow-hidden relative">
                        <div className="flex whitespace-nowrap animate-marquee">
                            {(messages && messages.length > 0 ? [...messages, ...messages] : ["ยินดีต้อนรับสู่ YouOKE TV", "สแกนเพื่อเริ่มร้องเพลง"]).map((msg, i) => (
    ...
    // Ads Template Fallback
    return (
        <div className="absolute inset-0 bg-stone-950 overflow-hidden flex flex-col font-sans select-none text-white">
            {renderBackground(100)}
            <div className="absolute bottom-[5vh] left-[5vw] z-20 flex items-center gap-[2vw] bg-black/60 backdrop-blur-2xl border border-white/10 p-[1.5vw] rounded-[2rem]">
                <div className="bg-white p-[0.8vw] rounded-2xl">
                    <QRCodeSVG value={qrUrl} size={150} level="M" className="w-[7vw] h-[7vw]" />
                </div>
                <div>
                    <p className="text-white/40 text-[0.8vw] font-bold uppercase mb-1">Join Party</p>
                    <div className="flex items-center gap-[1vw]">
                        <span className="text-[3vw] font-black text-white leading-none">{roomCode}</span>
                        <div className="w-[1px] h-[3vw] bg-white/20" />
                        <span className="text-primary font-bold text-[1vw]">youoke.tv</span>
                    </div>
                </div>
            </div>
        </div>

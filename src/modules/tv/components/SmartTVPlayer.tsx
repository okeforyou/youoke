                ...
                <div className="flex items-end gap-6 max-w-5xl">
                    ...
                    <div className="pb-1 min-w-0 flex-1">
                        <div className="flex items-center gap-3 mb-2">
                            <span className="bg-primary px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider text-white shadow-lg shadow-primary/40 animate-pulse">Now Playing</span>
                            {currentVideo.addedBy && (
                                <span className="flex items-center gap-1.5 text-xs font-medium text-white/70 bg-black/40 px-2 py-0.5 rounded-full border border-white/5 backdrop-blur-sm">
                                    <UserIcon className="w-3 h-3 text-white/50" />
                                    {currentVideo.addedBy.displayName || currentVideo.addedBy.name || 'Guest'}
                                </span>
                            )}
                        </div>
                        <h2 className="text-3xl font-bold text-white leading-tight drop-shadow-md truncate pr-10 tracking-tight font-display">{currentVideo.title}</h2>
                        <p className="text-lg text-white/60 mt-1 font-medium truncate">{currentVideo.author}</p>
                    </div>
                </div>
            </div>

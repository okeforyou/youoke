...
export const CHANGELOGS = [
    ...
    },
    {
        ...
        date: "2026-09-07",
        changes: [
            "fix(mixer): Added comprehensive resetMixer() to useMixerStore and connected it to top header 'รีเซ็ต' button across both Fullscreen Mixer and Sidebar Mixer modals.",
            "fix(fullscreen): Resolved missing resetPitchAndSpeed/resetMixer store destructuring in FullscreenControlBar, allowing one-click instant reset of pitch (ORIG), speed (1.0x), track volumes (100%), and un-muting."
        ]
    },
    {
        version: "5.5.345",
    ...
    },
    {
        ...
        date: "2026-09-06",
        changes: [
            "fix(player): Eliminated destructive continuous currentTime seek snapping in UniversalPlayer, replacing it with imperceptible playbackRate micro-adjustments (+-1%) to maintain stem phase lock without audio buffer flushes or stutter.",
            "fix(player): Isolated hard currentTime seeks strictly to initial track start and large timeline scrubbing (>1.5s), making vocal/music muting 100% smooth and continuous."
        ]
    },
    {
        version: "5.5.327",
    ...
    },
    {
        ...
        date: "2026-08-27",
        changes: [
            "fix(core): Resolved an issue where AI vocal separation was auto-triggered unconditionally on all YouTube dashboard and Music Station song clicks, causing unnecessary server requests and UI stuck states.",
            "fix(core): Bound the `aiVocalRequested` flag to only trigger automatically when the active search mode is 'ai_karaoke' (AI Studio Karaoke), keeping normal song/MV playback clean."
        ]
    },
    {
        version: "5.5.307",
        date: "2026-08-27",
        changes: [
            "fix(ui): Resolved a critical UX bug where the 'ตัดเสียงร้อง' (Vocal Separation) button disappeared completely from the queue items if a previous job was in 'error', 'processing', or 'idle' state in localStorage.",
            "fix(ui): Always render active progress bar or detailed error message regardless of client session's aiVocalRequested flag, preventing UI status desyncs.",
            "feat(ui): Added a direct 'ลองใหม่' (Retry) button to the error state in the queue items, allowing users to restart failed jobs instantly."
        ]
    },
    {
        version: "5.5.306",
    ...
    },
    {
        ...
        date: "2026-08-14",
        changes: [
            "feat(ai-vocal): Implemented strict concurrency=1 queue system in Python bridge to process vocal separation 1 song at a time to prevent CPU/RAM overload.",
            "feat(ai-vocal): Added Pause and Resume support for AI vocal separation jobs (via /pause and /resume endpoints and plugin dashboard).",
            "fix(ai-vocal): Fixed cache deletion issue where deleted/error songs persisted in Zustand store (localStorage) or bridge cache.",
            "ui(dashboard): Added light/dark mode system theme adaptation and fixed Electron launch background flicker."
        ]
    },
    {
        version: "5.5.297",

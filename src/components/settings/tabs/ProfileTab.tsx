...
export default function ProfileTab({ onClose }: { onClose: () => void }) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-300 max-w-3xl pb-8">
            
            {/* Section 1: Settings (Top) */}
            <div className="space-y-3">
                <h3 className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest pl-4">การตั้งค่าการแสดงผล</h3>
                <div className="bg-white dark:bg-zinc-950 rounded-3xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
                    <div className="p-4 sm:p-5 flex items-center justify-between transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h4 className="text-sm font-bold text-zinc-900 dark:text-white leading-none mb-1">โหมดหน้าจอ (Theme)</h4>
                                <p className="text-[11px] font-medium text-zinc-500">เลือกรูปแบบการแสดงผล</p>
                            </div>
                        </div>
            ...
            <div className="space-y-3">
                <h3 className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest pl-4">โปรไฟล์บัญชี</h3>
                <div className="bg-white dark:bg-zinc-950 rounded-3xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm p-6 relative">
                    
                    {/* Logout Button (Top Right) */}
                    <button
                        onClick={handleLogout}
                        className="absolute top-6 right-6 w-10 h-10 flex items-center justify-center rounded-xl bg-rose-50 dark:bg-rose-500/10 text-rose-500 border border-rose-100 dark:border-rose-900/50 hover:bg-rose-100 dark:hover:bg-rose-500/20 hover:text-rose-600 transition-all shadow-sm"
                        title="ออกจากระบบ"
                    >
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-center md:items-start text-center md:text-left">
                        {/* Avatar */}
                        <div className="w-20 h-20 rounded-full overflow-hidden bg-zinc-100 dark:bg-zinc-800 border-4 border-white dark:border-zinc-900 shadow-xl flex items-center justify-center shrink-0">
                            {user?.photoURL ? (
                        ...
                        <div className="flex-1 mt-2 md:mt-0 w-full">
                            ...
                            <div className="mt-3 flex flex-wrap justify-center md:justify-start gap-2">
                                <span className="px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-widest bg-zinc-100 dark:bg-zinc-800/80 text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700/50 shadow-sm">
                                    {user?.role === 'admin' ? 'ADMIN' : 'MEMBER'}
                                </span>
                            </div>

                            {/* LINE Connect (Inside Profile) */}
                            <div className="mt-6 pt-6 border-t border-zinc-100 dark:border-zinc-800/60 flex flex-col sm:flex-row items-center justify-between gap-4">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-[#06C755]/10 text-[#06C755] border border-[#06C755]/20">
                                    ...
                                    <div className="text-left">
                                        <h4 className="text-sm font-bold text-zinc-900 dark:text-white leading-none mb-1">บัญชี LINE</h4>
                                        <p className="text-[11px] font-medium text-zinc-500">เชื่อมต่อเพื่อรับการแจ้งเตือน</p>
                                    </div>
                                </div>
                                ...
                                {!isLineConnected ? (
                                    <button 
                                        ...
                                        className="px-4 py-2 rounded-xl bg-[#06C755] hover:bg-[#06C755]/90 text-white text-xs font-bold transition-all shadow-sm shrink-0 w-full sm:w-auto"
                                    >
                                        เชื่อมต่อ LINE
                                    </button>
                                ) : (
                                    <span className="px-3 py-1.5 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold text-[10px] uppercase tracking-wider shrink-0 border border-emerald-200 dark:border-emerald-500/20">
                                        เชื่อมต่อแล้ว
                                    </span>
                                )}
                            </div>
                            ...
                            {showLineQR && !isLineConnected && (
                                <div className="mt-4 p-6 bg-zinc-50 dark:bg-zinc-900/30 rounded-2xl flex flex-col items-center justify-center text-center animate-in zoom-in-95 duration-300 border border-zinc-100 dark:border-zinc-800">
            ...
            <div className="space-y-3">
                <h3 className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest pl-4">แพ็กเกจ (Membership)</h3>

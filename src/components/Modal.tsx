    ...
    <>
      ...
      >
        ...
        >
          {/* Header */}
          {title ? (
            <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-white shrink-0 z-20">
              {typeof title === 'string' ? <h3 className="text-xl font-bold text-gray-900">{title}</h3> : <div className="w-full pr-8">{title}</div>}
              <button
                onClick={() => {
                  setShowModal(false);
                  if (onClose) onClose();
            ...
            <button onClick={() => {
              setShowModal(false);
              if (onClose) onClose();
            }} className="z-50 btn btn-sm btn-circle bg-white/80 hover:bg-white text-gray-900 border-none shadow-sm absolute right-4 top-4 backdrop-blur-md">✕</button>
          )}
          ...
          {footer && (
            <div
              className="p-6 bg-white shrink-0 z-10 w-full"

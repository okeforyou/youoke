...
interface UserSummary {
    ...
    displayName: string;
    email: string;
    installed_modules?: string[]; // Made optional to be safe
}

export default function AdminModulesPage() {
    const [users, setUsers] = useState<UserSummary[]>([]);
    const [loading, setLoading] = useState(true);
    const [selectedModule, setSelectedModule] = useState(MODULES[0] ? MODULES[0].id : '');
    ...
    const toggleUserModule = async (userId: string, moduleId: string, hasModule: boolean) => {
        try {
            ...
            setUsers(prev => prev.map(u => {
                if (u.uid === userId) {
                    const mods = u.installed_modules || [];
                    return {
                        ...u,
                        installed_modules: hasModule
    ...
    // Filter out core modules if needed, or show all
    const displayModules = MODULES.filter(m => m.category !== 'core');
    const currentModule = MODULES.find(m => m.id === selectedModule) || displayModules[0];

    // Ensure initial selection if empty and modules exist
    useEffect(() => {
        if (!selectedModule && displayModules.length > 0) {
            setSelectedModule(displayModules[0].id);
        }

            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            className="w-full h-11 rounded-lg border border-gray-200 bg-transparent px-4 text-black outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                    ...
                    <div className="space-y-2">
                        ...
                        <select
                            ...
                            className="w-full h-11 rounded-lg border border-gray-200 bg-transparent px-4 py-2 outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                        >
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex gap-4">
                        <label className={cn(
                            ...
                            formData.type === 'html' ? "border-primary bg-primary/5 ring-1 ring-primary" : "border-gray-200 hover:bg-gray-50"
                        )}>
                            <input type="radio" className="hidden" name="type" checked={formData.type === 'html'} onChange={() => setFormData({ ...formData, type: 'html' })} />
                            <DocumentTextIcon className={cn("w-5 h-5", formData.type === 'html' ? "text-primary" : "text-gray-400")} />
                            <span className="font-medium">เนื้อหาข้อความ (HTML)</span>
                        </label>
                        <label className={cn(
                            "flex items-center gap-3 p-3 border rounded-lg cursor-pointer transition-all w-full md:w-auto",
                            formData.type === 'link' ? "border-primary bg-primary/5 ring-1 ring-primary" : "border-gray-200 hover:bg-gray-50"
                        )}>
                            <input type="radio" className="hidden" name="type" checked={formData.type === 'link'} onChange={() => setFormData({ ...formData, type: 'link' })} />
                            <GlobeAltIcon className={cn("w-5 h-5", formData.type === 'link' ? "text-primary" : "text-gray-400")} />
                ...
                {formData.type === 'html' ? (
                    <div className="space-y-2">
                        ...
                        <p className="text-xs text-gray-500 mt-2">พิมพ์เนื้อหาจัดรูปแบบได้ตามต้องการ</p>
                    </div>
                ) : (
                    <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                        ...
                        <input
                            ...
                            className="w-full h-11 rounded-lg border border-gray-200 bg-transparent px-4 outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                ...
                <div className="pt-4 border-t border-gray-200 flex justify-end">
                    <button
                        type="submit"
                        disabled={loading}
                        className="flex items-center gap-2 bg-primary text-white px-6 py-2.5 rounded-lg hover:bg-primary/90 transition-all shadow-md disabled:opacity-50"
                    >
                        {loading ? <ArrowPathIcon className="w-5 h-5 animate-spin" /> : <FolderArrowDownIcon className="w-5 h-5" />}

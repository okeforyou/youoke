...
export const useDualScreenSender = (onNextRequest?: () => void) => {
    ...
    useEffect(() => {
        ...
        const handler = (event: MessageEvent) => {
            if (event.data?.type === 'REQUEST_STATE') {
                ...
                console.log('📡 [Sender] Sent SYNC_STATE:', payload.payload);
            } else if (event.data?.type === 'REQUEST_NEXT') {
                console.log('📡 [Sender] Received REQUEST_NEXT');
                if (onNextRequest) onNextRequest();
            }
        };
    ...
    const play = useCallback(() => {
        if (!channelRef.current) return;
        channelRef.current.postMessage({ type: 'PLAY', videoId: curVideoIdRef.current });
    }, []);

    const pause = useCallback(() => {
        if (!channelRef.current) return;
        channelRef.current.postMessage({ type: 'PAUSE', videoId: curVideoIdRef.current });
    }, []);

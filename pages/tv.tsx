...
const TVPage = () => {
    ...
    const { roomCode, state, isConnected, setState, mode } = useReceiverLogic(player);
    ...
    useEffect(() => {
        ...
        // Sync Mute (Default to unmute for TV if possible, but browser policy restricts)
        // We'll trust state.controls.isMuted
    ...
    const upcomingQueue = state.queue.slice(state.currentIndex + 1); // Get next songs
    ...
    const onStateChange = (e: any) => {
        ...
        if (e.data === 0) {
            ...
            if (nextIndex < state.queue.length) {
                ...
                // Update DB logic needs to be in the hook, but for now this is visual
                // In real implementation, the hook should expose a 'nextTrack' function that updates DB
                ...
                {currentVideo ? (
                    <div className="w-full h-full relative">
                        <YouTube
                            ...
                            className="w-full h-full pointer-events-none" // Pointer events none prevents interaction with YT iframe
                            onReady={(e) => {
                                setPlayer(e.target);
                                e.target.mute(); // Auto-play policy often requires mute first
                            }}
                            ...
                            <div className="flex gap-8 items-end">
                                ...
                                <div className="flex-1 mb-2">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <span className="flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-gray-300 text-xs font-bold uppercase tracking-wider">
                                            {state.controls.isMuted ? <SpeakerXMarkIcon className="w-3 h-3" /> : <SpeakerWaveIcon className="w-3 h-3" />} Volume
                                        </span>
                                    </div>

...
- [x] Fix overlapping bug where all volume sliders rendered simultaneously by replacing opacity-based transitions with `hidden group-hover:block` display control.
...
- [x] `src/modules/player/components/FullscreenControlBar.tsx`: Move all stem tracks inline with CSS hover-expandable volume sliders positioned above the buttons with display-none default state, and add lyrics 3-pill segment selector.
